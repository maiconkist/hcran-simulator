...
def dump_res():
    ...
    good_cap = len(g_list)
    # avg of good_caps
    regex.compile("per_used:([0-9]*\.[0-9]*)")
    good_cap_avg_used = [float(i) for i in regex.findall("\n".join(g_list))] / len(g_list)
    ...
    bad_cap_avg_used = [float(i) for i in regex.findall("\n".join(g_list))] / len(g_list)
...
if __name__ == '__main__':
    ...
    res_str = "ue rrh it conn dis bbu_ch bw_update bw_max good_cap bad_cap avg_rbs_used\n"
    n_ue = 0
    n_ue = 0
    grid = None
